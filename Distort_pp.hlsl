...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	float2 distortVector = float2(distortTexture.g, distortTexture.b);	/*FILTER - not 0, similar to crinkle vector used in burn shader above*/
	...
	float3 outputColour = light + SceneTexture.Sample( PointSample, input.uv + gDistortLevel * distortVector ).rgb;
