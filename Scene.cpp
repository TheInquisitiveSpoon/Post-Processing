...
float gDegrees = 0.0f;
const float gDegreesCutOff = 10.0f;
float gWaterWaveSpeed = 50.0f;
...
bool InitGeometry()
{
	...
	if (!LoadTexture("Stars.jpg",                &gStarsDiffuseSpecularMap,  &gStarsDiffuseSpecularMapSRV) ||
		...
		!LoadTexture("Brick1.jpg",				 &gWall1DiffuseSpecularMap, &gWall1DiffuseSpecularMapSRV) ||
		!LoadTexture("Brick_35.jpg",			 &gWall2DiffuseSpecularMap, &gWall2DiffuseSpecularMapSRV))
	{
...
bool InitScene()
{
	...
	gWall1->SetPosition({ 42, 5, -10 });
	gWall1->SetRotation({ 0.0f, ToRadians(-110.0f), 0.0f });
	gWall1->SetScale(1.5f);
...
void RenderSceneFromCamera(Camera* camera)
{
	...
	gD3DContext->PSSetShaderResources(0, 1, &gWall1DiffuseSpecularMapSRV);
	gWall1->Render();
...
void PostProcessing(float frameTime)
{
	...
	if (gCurrentPostProcess == PostProcess::Tint)
	{
		...
		gPostProcessingConstants.tintColour = { 0, 0, 1 };
	...
	else if (gCurrentPostProcess == PostProcess::Blur)
	{
		...
		gPostProcessingConstants.viewportDimensions = { static_cast<float>(gViewportWidth), static_cast<float>(gViewportHeight) };
	}

	else if (gCurrentPostProcess == PostProcess::Underwater)
	{
		gD3DContext->PSSetShader(gUnderwaterPostProcess, nullptr, 0);
		gDegrees += gWaterWaveSpeed * frameTime;

		if (gDegrees >= gDegreesCutOff && gWaterWaveSpeed > 0.0f)
		{
			gWaterWaveSpeed = -gWaterWaveSpeed;
		}
		else if (gDegrees <= -gDegreesCutOff && gWaterWaveSpeed < 0.0f)
		{
			gWaterWaveSpeed = -gWaterWaveSpeed;
		}

		float rads = PI / 180.0f * gDegrees;
		gPostProcessingConstants.radians = rads;
		gPostProcessingConstants.waterColour = { 0, 0, 1 };
	}
